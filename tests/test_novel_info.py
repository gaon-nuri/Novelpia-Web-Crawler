...
from src.const.const import NOVEL_STATS_NAMED_TU as STATUSES
from src.func.userio import print_under_new_line
from src.parsers import novel_obj_from_page


class GetFinJamo(TestCase):
    def setUp(self):
        from src.const.const import SUFFIX_NAMED_TU, SUFFIX_NAMED_TU_CLS
        self.ko_word_v_end: str = "창작물 속으로"
        self.ko_word_c_end: str = "매도당하고 싶은 엘프님"
        self.suffix_tu: SUFFIX_NAMED_TU_CLS = SUFFIX_NAMED_TU

    from src.func.common import suffix_from_words

    @staticmethod
    def get_josa(kr_word: str, suffix: str):
        """suffix_from_words 함수 주석 참고"""
        return GetFinJamo.suffix_from_words(kr_word, suffix)

    def test_fin_cons_word(self):
        word: str = self.ko_word_c_end
        args: tuple = self.suffix_tu.cons
        answer_suffix = tuple(map(self.get_josa, [word] * 4, args))
        self.assertEqual(answer_suffix, args)

    def test_fin_vowel_word(self):
        word: str = self.ko_word_v_end
        args: tuple = self.suffix_tu.vowel
        answer_suffix = tuple(map(self.get_josa, [word] * 4, args))
        self.assertEqual(answer_suffix, args)
...
class GetLikeNovelInfo(TestCase):
    ...
    def toggle_novel_alarm(novel_code: str):
        """원 함수 주석 참조"""
        from src.user.shelf import pick_novel_act
        act_alarm: int = 1
        success, alarms = pick_novel_act(novel_code, act_alarm)
    ...
    def toggle_novel_like(novel_code: str):
        ...
        from src.user.shelf import pick_novel_act
        csrf: str = dotenv_values()["CSRF"]
        success, likes = pick_novel_act(novel_code, 1, True, csrf)
    ...
    @classmethod
    def novel_dic_gen_from_mem(cls, novel_cnt):
        """ 원 함수 주석 참고"""
        from src.user.shelf import novel_dic_gen_from_mem
        novel_dic_gen = novel_dic_gen_from_mem(cls.mem_no, novel_cnt)
        return novel_dic_gen

    def test_get_like_novel_info_dic(self):
        novel_code: str = "99"  # <눈송아리>
        self.toggle_novel_like(novel_code)
        novel_dic_gen = self.novel_dic_gen_from_mem(novel_cnt=1)
        self.toggle_novel_like(novel_code)
        try:
            got_dic: dict = next(novel_dic_gen)
        # 선호작 X
...
class GetNovelStatus(TestCase):
    ...
    def chk_up_status(novel_code: str) -> str:
        ...
        sub_mem: int = 1
        novel_gen, novel_cnt = novel_gen_from_mem(sub_mem, novel_code)
        assert novel_cnt == 1
        try:
            novel: Novel = next(novel_gen)
        except StopIteration:
            from urllib.parse import urljoin
            # from src.novel_info import parse_novel_page_up_status
            from src.func.crawl import fetch_page_from_url
            from src.const.const import BASE_URL

            # 메인 페이지 요청
            base_url: str = urljoin(BASE_URL, "/novel/")
            rel_url: str = novel_code
            abs_url: str = urljoin(base_url, rel_url)
            with fetch_page_from_url(abs_url) as (page, err):
                if err:
                    raise err
            novel: Novel = novel_obj_from_page(page)
    ...
    def test_ongoing_novel(self):
        ...
        self.assertEqual(None, up_status)
...
class CntNovelInStatus(GetNovelStatus):
    ...
    def test_cnt_novel_on_hiatus(self):
        """연중작 비율을 재는 테스트"""
        for num in range(1, self.ALL_NOVEL_CNT):
            code = str(num)
...
class NovelToMdFile(TestCase):
    ...
    def test_novel_to_md(self):
        ...
        from src.novel_info import novel_to_md_string
...
"""
    ...
    def test_novels_to_md_files(self):
        ...
        from src.const.const import BASE_URL, ALL_NOVEL_CNT
        ...
        for i in range(1, ALL_NOVEL_CNT):
            code = str(i)
            with self.subTest(url=urljoin(base_url, code)):
                novel: Novel = novel_obj_from_code(code)
                if not novel:
                    continue
